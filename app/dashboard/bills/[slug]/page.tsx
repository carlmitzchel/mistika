...
function fmt(centavos: number): string {
  return (
    "₱" + (centavos / 100).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "settling")
    return <span className="badge badge-pending">💰 Settling</span>;
  if (status === "settled")
...
function PaymentStatusBadge({
  ...
  if (status === "confirmed")
    return <span className="badge badge-settled">✅ Paid</span>;
  if (status === "rejected")
    return <span className="badge badge-rejected">❌ Rejected</span>;
  if (status === "proof_submitted")
    return <span className="badge badge-proof">📎 Proof sent</span>;
  if (status === "pending" && hasRecord)
    return <span className="badge badge-proof">💸 Marked paid</span>;
  return null;
}
...
export default function BillDetailPage() {
      ...
      if (paymentsRes.ok) {
        ...
        for (const p of billData.participants) {
          const payment = paymentsData.find(
            (pay: Payment) => pay.fromParticipantId === p.id,
          );
  ...
  const handleCloseBill = async () => {
    if (
      !window.confirm(
        "Are you sure you want to close this bill? It will be marked as settled.",
      )
    )
      return;
    ...
    if (res.ok) {
      setBill((prev) => (prev ? { ...prev, status: "settled" } : null));
    }
  };
  ...
  if (!bill) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <Link
          href="/dashboard"
          className="btn-doodle btn-coral mt-4 inline-block"
        >
          Back to dashboard
        </Link>
      </div>
  ...
  const allPaid =
    shares.length > 0 &&
    shares.every(
      (s) => s.balance <= 0 || paymentStatuses[s.participantId] === "confirmed",
    );
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="flex flex-col gap-3 mb-6">
          {shares.map((share, i) => {
            ...
            const discountTypeLabel = (type: string | null) => {
              if (type === "pwd")
                return (
                  <span className="badge" style={{ background: "#C4B5FD" }}>
                    🦽 PWD
                  </span>
                );
              if (type === "senior")
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  ...
                  <PaymentStatusBadge
                    status={payStatus}
                    hasRecord={!!paymentIds[share.participantId]}
                  />
                </div>
                ...
                <div className="flex items-center justify-between mt-2 flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {(payStatus === "proof_submitted" ||
                      (payStatus === "pending" &&
                        !!paymentIds[share.participantId])) && (
                      <>
                        <button
      ...
      <div className="flex gap-3 mt-6">
        {allPaid && bill.status !== "settled" && (
          <button
            onClick={handleCloseBill}
            className="btn-doodle btn-mint flex-1"
          >
            ✅ Close Bill
          </button>
        )}
        <button
          ...
          className="btn-doodle btn-ghost flex-1 text-[#ef4444]"
